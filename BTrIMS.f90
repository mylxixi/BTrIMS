...
CHARACTER(LEN=50), PARAMETER :: fwshed_era5 = "Scotlan_mask_int2.nc"
...
PROGRAM back_traj
	...
	INTEGER :: side,left,right,up,down
	...
	REAL :: ttfac,nnfac,precip_here,qfac_evap,qfac_pbl,grid_ratio,accumu_cont
...
!$OMP SCHEDULE (DYNAMIC)
				...
				do tt = 1, daytsteps
					...
					do mm = 1, par_release(tt)
						...
						do nn = totsteps-daytsteps+tt, 2, -1
							...
							!
							grid_ratio = cos(lat2d(x,y)*pi/180)/cos(lat2d(xx,yy)*pi/180)
							...
							!if (par_lev >= pbl_lev(x,y,nnMM5+1)) then	
								if (lin_interp(evap(x,y,nnMM5:nnMM5+1),nnfac) > 0.) then
									if (grid_ratio > 1) then
										...
										accumu_cont = accumu_cont + lin_interp(evap(x,y,nnMM5:nnMM5+1),nnfac)/(indatatsteps*lin_interp(tpw(x,y,nnMM5:nnMM5+1),nnfac))*qfac_evap
									else
										print*, 'must come to side grid'
										side = NINT((sqrt(1/grid_ratio)-1)/2)
										left = max(x-side,1)
										right = min(x+side,dim_j)
										down = max(y-side,1)
										up = min(y+side,dim_i)	
										print*,'side,left,right,down,up',side,left,right,down,up							
										WV_cont(left:right,down:up) = WV_cont(left:right,down:up) + lin_interp(evap(x,y,nnMM5:nnMM5+1),nnfac) &
												/ (indatatsteps*lin_interp(tpw(x,y,nnMM5:nnMM5+1),nnfac))*qfac_evap
										accumu_cont = accumu_cont + lin_interp(evap(x,y,nnMM5:nnMM5+1),nnfac)/(indatatsteps*lin_interp(tpw(x,y,nnMM5:nnMM5+1),nnfac))*qfac_evap
									end if
       ...
       !!! This sction needs modifying if splitting PBL
							!
							if (x<2) then
								WV_cont(1,y) = 1. - accumu_cont
								EXIT
							else if (x>dim_j-2) then
								WV_cont(dim_j,y) = 1. -  accumu_cont
								EXIT
							end if
							if (y<2) then
								WV_cont(x,1) = 1. -  accumu_cont
								EXIT
							else if (y>dim_i-2) then
								WV_cont(x,dim_i) = 1. -  accumu_cont
								EXIT
							end if
